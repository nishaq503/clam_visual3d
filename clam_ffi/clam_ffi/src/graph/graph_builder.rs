...
use crate::utils::types::Cakesf32;
use crate::{
    debug,
    // debug,
...
//     // dataset: &DataSet,
//     cakes: Option<Rc<RefCell<Cakesf32>>>,
//
//     // node_visitor: crate::CBFnNodeVisitorMut,
// ) -> Vec<Edge> {
//     if let Some(cakes) = cakes {
//         let cakes = cakes.borrow();
//         let tree = cakes.trees().first().unwrap();
//         let dataset = tree.data();
//         let mut edges: Vec<Edge> = Vec::new();
//         for i in 0..clusters.len() {
//             for j in (i + 1)..clusters.len() {
//                 let distance = clusters[i].distance_to_other(dataset, clusters[j]);
